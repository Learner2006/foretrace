    ...
    async def fetch_company_filing(self, company_name: str, ticker: Optional[str]) -> str:
        if ticker and self.is_indian_ticker(ticker):
            ...
            try:
                ...
                emergency_filing_cache[ticker.upper().strip()] = extracted
                return extracted
            except Exception as e:
                if ticker:
                    key = ticker.upper().strip()
                    if key in emergency_filing_cache:
                        logger.warning(
                            f"Failed to fetch market data for Indian ticker {ticker} ({e}). Using emergency fallback cache.",
                            extra={"metadata": {"event": "market_emergency_fallback_used", "ticker": ticker, "error": str(e)}}
                        )
                        return emergency_filing_cache[key]
                raise e
        ...
        try:
            ...
            if ticker:
                try:
                    ...
                    logger.warning(f"Failed to get CIK by ticker {ticker}: {e}")
            ...
            extracted = self.extract_signal_text(raw_doc)
            if ticker:
                emergency_filing_cache[ticker.upper().strip()] = extracted
            return extracted
        except Exception as e:
            if ticker:
                ...
                if key in emergency_filing_cache:
                    logger.warning(
                        f"Failed to fetch SEC filing for {ticker} ({e}). Using emergency fallback cached filing.",
                        extra={"metadata": {"event": "sec_emergency_fallback_used", "ticker": ticker, "error": str(e)}}
                    )
                    return emergency_filing_cache[key]
            raise e
